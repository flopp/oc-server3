...
if ($error == false)
{
	...
	{
		$error = false; // set if there were any errors
		...
		else if( is_numeric($search_time) )
		{
			$search_time_ok = true;
		}
		...
		{
			tpl_set_var('search_time', $search_time);
		}
		...
		if ($show_all_langs == 0)
		{
			...
			$rs = sql("SELECT `languages`.`short`, IFNULL(`sys_trans_text`.`text`, `languages`.`name`) AS `name` 
						FROM `languages` 
						...
						WHERE `languages_list_default`.`lang`='&1' 
		...
		else
		{
			...
			$rs = sql("SELECT `languages`.`short`, IFNULL(`sys_trans_text`.`text`, `languages`.`name`) AS `name` 
						FROM `languages` LEFT JOIN `sys_trans` ON `languages`.`trans_id`=`sys_trans`.`id` 
						LEFT JOIN `sys_trans_text` ON `sys_trans`.`id`=`sys_trans_text`.`trans_id` AND `sys_trans_text`.`lang`='&1' 
						ORDER BY `name` ASC", $locale);
		}
		...
		if ($show_all_countries == 0)
		{
			...
			$rs = sql("SELECT `countries`.`short`, IFNULL(`sys_trans_text`.`text`, `countries`.`name`) AS `name` 
						FROM `countries` 
		...
		while ($rAttrGroup = sql_fetch_assoc($rsAttrGroup))
		{
			...
			$rs = sql("SELECT `cache_attrib`.`id`, IFNULL(`ttname`.`text`, `cache_attrib`.`name`) AS `name`, `cache_attrib`.`icon_undef`, `cache_attrib`.`icon_large`, IFNULL(`ttdesc`.`text`, `cache_attrib`.`html_desc`) AS `html_desc` 
						...
						WHERE `cache_attrib`.`group_id`=" . ($rAttrGroup['id']+0) . " AND
							...
							`cache_attrib`.`selectable`!=0 
						ORDER BY `cache_attrib`.`group_id`, `cache_attrib`.`id`", $locale);
			while($record = sql_fetch_array($rs))
			{
		...
		if (isset($_POST['submitform']))  // Ocprop
		{
			...
			if ( ( $wp_gc != "" && !mb_ereg_match('^[Gg][Cc][0-9A-Za-z]{1,6}$', $wp_gc ) ) ||
				 ( $wp_nc != "" && !mb_ereg_match('^[Nn][0-9A-Za-z]{1,6}$', $wp_nc) ) )
			{
				tpl_set_var('wp_message', $wp_not_ok_message );
				$error = true;
			}
			
			//check hidden_since
			if ( !is_numeric($hidden_day) || !is_numeric($hidden_month) || !is_numeric($hidden_year) || 
				 !checkdate($hidden_month, $hidden_day, $hidden_year) )
			{
			...
			// only events and virtual caches may have "no container"
